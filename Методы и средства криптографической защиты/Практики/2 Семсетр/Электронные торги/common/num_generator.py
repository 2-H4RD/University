...
по детерминистическому методу (Процедура A), а также
обёртки для генерации простых чисел, которые можно
использовать, в том числе, как p и q для RSA-аутентификации.

Ключевые моменты:

- p0 = p — большое простое (~t бит),
- q = p1 — простое (~t/2 бит), делящее p-1,
...
- собственный SHA-256 (для получения 256-битных seed'ов),
- LCG (линейный конгруэнтный генератор) как основной PRNG.

ВНИМАНИЕ:
- Тест Миллера–Рабина и решето Эратосфена ИСКЛЮЧЕНЫ.
- Простота больших чисел p и q гарантируется самой процедурой A.
- Для начального маленького простого p_s (t_s <= 17 бит) используется
  простая проверка пробным делением.
"""
...
# ----------------------- Малое простое p_s (t_s <= 17 бит) ----------------------- #

def _is_prime_small(n: int) -> bool:
    """
    Простая проверка простоты для маленьких n (до 2^17).
    Используем прямое пробное деление: делители от 2 до floor(sqrt(n)).
    Это соответствует описанию для p_s в теоретической части.
    """
    if n < 2:
        return False
    if n in (2, 3):
    ...
    if n % 2 == 0:
        return False
    d = 3
    # для n <= 2^17 максимум ~362, поэтому это очень дёшево
    while d * d <= n:
        if n % d == 0:
            return False
        d += 2
...
def _generate_small_prime(bits: int = 16) -> int:
    ...
    Здесь:
      ...
      - проверяем простоту функцией _is_prime_small.
    ...
    while True:
        ...
        # делаем нечётным и гарантируем старший бит = 1
        n |= 1
        n |= 1 << (bits - 1)
...
def _procedure_A_step(pi_plus_1: int,
    ...
    """
    MOD16 = 1 << 16
    q = pi_plus_1  # чтобы совпадало с условием теоремы: p = q * N + 1

    # --- шаг 5: r_m = floor(t_m / 16) ---
    r_m = t_m // 16
    if r_m <= 0:
        r_m = 1  # защитный случай
...
def generate_gost_pq(bits_p: int = 512,
    ...
    """
    ГОСТ Р 34.10-94, Процедура A (адаптирована под Python):
    ...
    if bits_q is not None:
        # Можно использовать при отладке:
...
# ----------------------- Обёртки для использования в других модулях ----------------------- #

def generate_gost_prime(bits_p: int = 512) -> int:
    """
    Сгенерировать ОДНО большое простое p заданной битности
    по ГОСТ-процедуре A (через generate_gost_pq) и вернуть только p.

    Это удобно использовать, например, как источник простых для RSA.
    """
    p, _q = generate_gost_pq(bits_p=bits_p)
    return p


def generate_two_gost_primes(bits_p: int = 512) -> Tuple[int, int]:
    """
    Сгенерировать ДВА различных простых p1 и p2 одинаковой битности
    по ГОСТ-процедуре A.

    Планируется использовать такие пары (p1, p2) как (p, q)
    в RSA-аутентификации (тогда модуль n ≈ 2 * bits_p).
    """
    p1 = generate_gost_prime(bits_p)
    while True:
        p2 = generate_gost_prime(bits_p)
        if p2 != p1:
            break
    return p1, p2


# ----------------------- Демонстрация при запуске файла ----------------------- #
...
if __name__ == "__main__":
    ...
    print("\n[3] Генерация двух простых p1, p2 по ГОСТ для возможного использования в RSA...")
    p1, p2 = generate_two_gost_primes(bits_p=bits_p)
    print(f"p1 bitlen = {p1.bit_length()}, p2 bitlen = {p2.bit_length()}")
    print(f"p1 = {p1}")
    print(f"p2 = {p2}")
    print(f"p1 == p2 ? {p1 == p2}")
