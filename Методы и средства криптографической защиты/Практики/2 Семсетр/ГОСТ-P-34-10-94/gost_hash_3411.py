...
# Шифр Магма вынесен в модуль gost_28147_89.py
...
C3_LE = bytes.fromhex("FF00FFFF000000FFFF0000FF00FFFF0000FF00FF00FF00FFFF00FF00FF00FF00")[::-1]

def _gen_keys(H_le: bytes, V_le: bytes) -> Tuple[List[int], List[int], List[int], List[int]]:
    """Генерация K1..K4 на LE-входах U=H_le, V=V_le."""
    U = H_le
    V = V_le
    W = bytes(a ^ b for a, b in zip(U, V)); K1 = _P_permutation(W)
    U = _A_transform_LE(U); V = _A_transform_LE(_A_transform_LE(V))
    W = bytes(a ^ b for a, b in zip(U, V)); K2 = _P_permutation(W)
    U = _A_transform_LE(U); U = bytes(a ^ b for a, b in zip(U, C3_LE))
    V = _A_transform_LE(_A_transform_LE(V))
    W = bytes(a ^ b for a, b in zip(U, V)); K3 = _P_permutation(W)
    U = _A_transform_LE(U); V = _A_transform_LE(_A_transform_LE(V))
    W = bytes(a ^ b for a, b in zip(U, V)); K4 = _P_permutation(W)
    return K1, K2, K3, K4
...
def _add_256_le(a: bytes, b: bytes) -> bytes:
    ...
    return ((int.from_bytes(a, "little") + int.from_bytes(b, "little")) & ((1<<256)-1)).to_bytes(32, "little")
