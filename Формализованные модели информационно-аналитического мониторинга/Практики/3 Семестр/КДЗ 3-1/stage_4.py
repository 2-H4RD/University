...
def print_first_elite_points(
    ...
    print(f"Первые {min(limit, len(elite_points))} точек из массива all_elites (Phi = 1):")
...
def _plot_population_xy_on_axis(
    axis,
    population: List[st1.Individual],
    title: str,
    x1_bounds: Tuple[float, float],
    ...
    pop_x, pop_y = st1.get_xy(
        population,
        x1_bounds=x1_bounds,
        x2_bounds=x2_bounds,
    )
    axis.scatter(pop_x, pop_y, s=16, alpha=0.9, color="blue", zorder=2)
...
def plot_threshold_population_pairs(
    ...
    """
    Для каждого порога строит ОТДЕЛЬНОЕ окно из двух графиков:
    - слева популяция в пространстве f1, f2
    - справа та же популяция в пространстве x1, x2
    """
    for threshold in thresholds:
        ...
        if snapshot is None:
            fig.suptitle(f"Порог {threshold * 100:.0f}% не достигнут")
            for axis, xlabel, ylabel in (
                (ax_obj, "f1 (min)", "f2 (min)"),
                (ax_xy, "x1", "x2"),
            ):
                axis.grid(True, alpha=0.3)
        ...
        title_suffix = (
            ...
            f"итерация {snapshot['iteration']} | "
            f"элитных {snapshot['elite_ratio'] * 100:.2f}%"
        )
        ...
        ax_obj.set_title(f"Популяция в пространстве f1, f2 {title_suffix}")
        ax_obj.set_xlabel("f1 (min)")
        ax_obj.set_ylabel("f2 (min)")
        ax_obj.grid(True, alpha=0.3)

        _plot_population_xy_on_axis(
            ax_xy,
            population=population,
            title=f"Та же популяция в пространстве x1, x2 {title_suffix}",
            x1_bounds=x1_bounds,
            x2_bounds=x2_bounds,
        )
...
def plot_all_elites_in_decision_space(
    all_elites: List[st1.Individual],
    x1_bounds: Tuple[float, float],
    x2_bounds: Tuple[float, float],
) -> None:
    if not all_elites:
        raise ValueError("Массив all_elites пуст")
    ...
    plt.scatter(
        ...
        edgecolors="black",
        linewidths=0.25,
        label="все накопленные элитные точки",
    )
    plt.title("Этап 4: все накопленные элитные точки в пространстве x1, x2")
    plt.xlabel("x1")
    plt.ylabel("x2")
    plt.grid(True, alpha=0.3)
    plt.legend()
...
def run_stage4(
    ...
    if iteration == max_iterations and history[-1]["elite_ratio"] < target_elite_ratio:
        print(
            f"Предупреждение: достигнут предел MAX_ITERATIONS={max_iterations}, "
...
if __name__ == "__main__":
    ...
    print("СТАДИЯ 4: циклическая эволюция до достижения целевой доли элитных точек")
    ...
    plot_all_elites_in_decision_space(
        all_elites,
        x1_bounds=(st1.X1_L, st1.X1_H),
        x2_bounds=(st1.X2_L, st1.X2_H),
    )
