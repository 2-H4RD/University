...
def select_parents_from_group(
    ...
    статической рулетки БЕЗ возвращения.

    После каждого выбора выбранная особь исключается из дальнейшего
    рассмотрения, а интервалы рулетки пересчитываются заново.
    Это уменьшает многократное дублирование одной и той же особи
    внутри группы на одном цикле отбора.
    """
    if not group:
        raise ValueError("Группа пуста.")
    if parents_per_group <= 0:
        raise ValueError("parents_per_group должен быть > 0.")
    if parents_per_group > len(group):
        raise ValueError(
            "parents_per_group не может превышать размер группы "
            "при отборе без возвращения."
        )
    ...
    available_indices = list(range(len(group)))
    selected_parents: List[st1.Individual] = []

    for _ in range(parents_per_group):
        current_fitness = [fitness_list[index] for index in available_indices]
        intervals = build_static_roulette_intervals(current_fitness)
        chosen_local_index = choose_member_by_static_roulette(intervals)
        chosen_group_index = available_indices.pop(chosen_local_index)

        _, individual = group[chosen_group_index]
